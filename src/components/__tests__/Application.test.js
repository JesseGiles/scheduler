...
describe("Application", () => {
  ...
  it("loads data, books an interview and reduces the spots remaining for the first day by 1", async () => {
    ...
    expect(getByText(appointment, "Saving humanity...")).toBeInTheDocument();
  ...
  it("loads data, cancels an interview and increases the spots remaining for Monday by 1", async () => {
    ...
    expect(getByText(appointment, "Choose wisely..")).toBeInTheDocument();

    // 5. Click the "Confirm" button on the confirmation.
    fireEvent.click(queryByText(appointment, "Confirm"));

    // 6. Check that the element with the text "Deleting" is displayed.
    expect(getByText(appointment, "Terminating...")).toBeInTheDocument();
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 5. Click the "Save" button on the after editing the interview.
    fireEvent.click(queryByText(appointment, "Save"));

    // 6. Check that the element with the text "Saving" is displayed.
    expect(getByText(appointment, "Saving humanity...")).toBeInTheDocument();

    // 7. Wait until the element with the Edit button is displayed again.
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    // 4. Enter the name "Jack Burton" into the input with the placeholder "Enter Student Name".
    fireEvent.change(getByPlaceholderText(appointment, /Enter Student Name/i), {
      target: { value: "Jack Burton" },
    });
    ...
    await waitForElement(() =>
      getByText(appointment, "Something done got borked")
    );
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    await waitForElement(() =>
      getByText(appointment, "I cant let you do that.")
    );
